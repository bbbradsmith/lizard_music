...
def generate_asm_strings():
    s = "; automatically generated by music_export.py\n"
    s += "; " + now_string + "\n"
    s += "\n"
    for i in range(0,len(all_titles)):
        s += "data_music_title_%d: .asciiz \"%s\"\n" % (i, all_titles[i])
    s += "\n"
    for i in range(0,len(all_sfx_titles)):
        s += "data_sfx_title_%d: .asciiz \"%s\"\n" % (i, all_sfx_titles[i])
    s += "\n"
    s += "data_music_titles:\n"
    for i  in range(0,len(all_titles)):
        s += ".addr data_music_title_%d\n" % i
    s += "\n"
    s += "data_sfx_titles:\n"
    for i  in range(0,len(all_sfx_titles)):
        s += ".addr data_sfx_title_%d\n" % i
    s += "\n"
    s += "; end of file\n"
    return s    
...
music_files = gather_files(MUSIC_INPUT_DIR,".ftm")
sfx_files = gather_files(SFX_INPUT_DIR,".ftm")

for i in range(len(music_files)):
    filename = music_files[i]
    ftm_file = os.path.join(MUSIC_INPUT_DIR,filename)
    txt_file = os.path.join(OUTPUT_DIR,"music__" + filename + ".txt")
    log_file = os.path.join(OUTPUT_DIR,"music__" + filename + ".log")
    ...
    if (len(e) > 0):
        ...
        error_log += e
    all_titles.append(ftm.title.upper().replace(" ","_"))
    all_speeds.append(ftm.speed)
    all_pattern_lengths.append(ftm.pattern_length)
    all_orders.append(po)
    all_patterns.append(pp)

for i in range(len(sfx_files)):
    filename = sfx_files[i]
    ftm_file = os.path.join(SFX_INPUT_DIR,filename)
    txt_file = os.path.join(OUTPUT_DIR,"sfx__" + filename + ".txt")
    log_file = os.path.join(OUTPUT_DIR,"sfx__" + filename + ".log")
    if not os.path.exists(ftm_file):
        break
    print ("Processing: "+ftm_file)
    if not skip_text_export:
    ...
    if (len(e) > 0):
        ...
        error_log += e
    all_sfx.append(sfx)
    all_sfx_titles.append(ftm.title.upper().replace(" ","_"))
...
f = open(os.path.join(OUTPUT_DIR,"data_music_strings.inc"), "wt")
f.write(generate_asm_strings())
f.close()
